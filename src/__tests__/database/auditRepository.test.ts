...
describe('logAuditEvent', () => {
  it('inserts a row with the correct action, resourceType, and resourceId', async () => {
    await logAuditEvent('captured', 'evidence', 'ev-001');
    expect(auditRows).toHaveLength(1);
    expect(auditRows[0].action).toBe('captured');
  ...
  it('assigns a unique UUID to each event', async () => {
    await logAuditEvent('captured', 'evidence', 'ev-004');
  ...
  it('logs different resource types', async () => {
    await logAuditEvent('uploaded', 'court_order', 'co-001');
...
describe('getAuditLog', () => {
  beforeEach(async () => {
    ...
    await logAuditEvent('captured',    'evidence',    'ev-002');
    await logAuditEvent('uploaded',    'court_order', 'co-001');
  ...
  it('filters by resourceType', async () => {
    ...
    expect(entries[0].action).toBe('uploaded');
  ...
  it('filters by both resourceType and resourceId', async () => {
    ...
    // captured, viewed, transcribed — all for ev-001/evidence
  ...
  it('maps snake_case columns to camelCase properties', async () => {
    ...
    expect(entry.action).toBe('captured');
...
describe('getAuditLogForEvidence', () => {
  it('returns only entries for the specified evidence item', async () => {
    await logAuditEvent('captured',    'evidence',    'ev-A');
    await logAuditEvent('viewed',      'evidence',    'ev-A');
    await logAuditEvent('captured',    'evidence',    'ev-B');
    await logAuditEvent('uploaded',    'court_order', 'co-X');
